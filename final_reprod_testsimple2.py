...
def simul_weight(time_ms, time_min, time_max, funct):
    #param for updating
    delta_up=5
    alpha=50
    ...
    plt.plot(np.linspace(0,time_ms,len(e_m_array)), np.array(wup_array)*10)
    plt.xlabel('Time (ms)')
    plt.ylabel('error')
    plt.xlim([0,time_ms])
    plt.axvline(time_min)
...
def f_triangle(time_ms):
    ...
    return ts
f_per = lambda x : np.cos(x/100) + np.sin(x/25)*.5
f_per_comp = lambda x : np.cos(x/500) + np.sin(x/125)*.5 + np.cos(x/50)*.3 + np.cos(x/200)*.2

def f_noise(time_ms):
    ts=list()
    for i in range(time_ms):
        ts.append(f_per(i) + np.random.normal(0,.1))
    return ts
